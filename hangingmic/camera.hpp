...
void Camera::rotaciona(GLdouble hAngle, GLdouble vAngle) {
	...
	if (hAngle != 0) {
		...
		frente[0] = c*frenteCopia[0] + s*direita[0];
		frente[1] = c*frenteCopia[1] + s*direita[1];
		frente[2] = c*frenteCopia[2] + s*direita[2];

		direita[0] = -s*frenteCopia[0] + c*direita[0];
		direita[1] = -s*frenteCopia[1] + c*direita[1];
		direita[2] = -s*frenteCopia[2] + c*direita[2];
...
void Camera::move(int deFrente, int deLado) {
	...
	if (deFrente) {
		posicao[0] += deFrente*frente[0];
		posicao[1] += deFrente*frente[1];
		posicao[2] += deFrente*frente[2];
	}
